...
pulsesequence()
{
  ...
  double  spoilMoment, mygradamp, mygraddur,pcro,pcpe,pcsl,megdelay,sheardelay;
  int     table,shapeEx;
  int     sepSliceRephase;
  char    spoilflag[MAXSTR],mriout[MAXSTR],slprofile[MAXSTR],mygradshape[MAXSTR];
  ...
  mygradamp = getval("mygradamp");
  mygraddur = getval("mygraddur");
  pcro = getval("pcro");
  pcpe = getval("pcpe");
  pcsl = getval("pcsl");
  megdelay = getval("megdelay");
  sheardelay = getval("sheardelay");
  //getstr("mygradshape",mygradshape); //MEP 150428
  //mygradshape = "bipolar";
  getstr("mygradshape",mygradshape);
  ...
  /* Min TE *********************************************/
  tetime = ss_grad.rfCenterBack + pe_grad.duration + alfa + ro_grad.timeToEcho + mygraddur + (2*megdelay)+sheardelay;
  ...
  /* Min TR *********************************************/   	
  trmin  = ss_grad.duration + te_delay + pe_grad.duration + ro_grad.duration + perTime + 8e-6 + mygraddur + (2*megdelay)+sheardelay;
...
/* START MEP 150428 */
      /* Motion Encoding Gradient ------- added DCC/MEP 04.28.15 */
/*
      // create gradient shape (bipolar)
      int nsteps = (int) floor(mygraddur/(4e-6));
      ...
      Gpattern gshape[512];
      for (j=0; j<nsteps; j++) {
	  gshape[j].amp = 32767*sin((double)j/50.0);
	  gshape[j].time = 1.0;
      }
      init_gradpattern("bipolar",gshape,nsteps);
*/
      // assign gradient direction
//      delay(megdelay);
//      sp2on();        // Scope trigger  */ // MEP MIGHT NEED TO ADD DELAY BETWEEN GRADIENTS!
//      delay(sheardelay);
//      obl_shapedgradient(mygradshape,mygraddur,pcro*mygradamp,pcpe*mygradamp,pcsl*mygradamp,WAIT);
//      obl_shapedgradient(mygradshape,mygraddur,-pcro*mygradamp,-pcpe*mygradamp,-pcsl*mygradamp,WAIT);
//      sp2off();
//      delay(megdelay);
/* STOP MEP 150428 */
